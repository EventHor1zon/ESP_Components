...
ESP_EVENT_DEFINE_BASE(CBUFF_EVENT_BASE);
#include "esp_event.h"
#endif
...
CBuff cbuffer_create(CBuffer_init_t *init)
{
    ...
    if (err == ESP_OK) {
        ...
        if (sem == NULL) {
            ...
            handle->sem = sem;
            // ESP_LOGI("CBUFF", "Created new CBuffer Size: %lu at 0x%08x", handle->buffer_len,
            // (uint32_t)buffer);
        }
    }
...
esp_err_t cbuffer_config_events(
    CBuff handle,
    bool use_events,
...
esp_err_t cbuffer_config_packet(
    CBuff handle,
    bool use_sep,
    uint8_t *sep_bytes,
    uint8_t sep_length,
...
esp_err_t cbuffer_write(CBuff handle, void *data, uint32_t wrt_len)
{
    ...
    if (!ret) {
        ...
        if (handle->use_events && handle->event_settings.event_mask & CBUFF_EVENT_OVERWRITE
...
esp_err_t cbuffer_write_packet(CBuff handle, void *data, uint32_t length)
{
    ...
    if (length != handle->pkt_settings.packet_data_sz_bytes) {
        // ESP_LOGE(CBUFF_TAG, "Error, invalid length! Expected %u got %u",
    ...
    // ESP_LOGI(CBUFF_TAG, "Wrote %u byte packet to cbuffer, %u free bytes left", length,
    // free_bytes);
            ...
            || (!pkt_dispatch && buffer_unread_bytes(handle) >= task->chunk_sz))
        {
            ...
            switch (task->io_type) {
                ...
                case CBUFF_IO_TYPE_UART:
                    ...
                    break;
                default: ESP_LOGE(CBUFF_TAG, "Error, invalid IO Type"); break;
            }
        }
...
static void cbuff_receive_task(void *args)
{
    ...
    while (1) {
        ...
        if (xQueueReceive(cbuff_read_queue, &rT, portMAX_DELAY) != pdTRUE) {
            ESP_LOGE(CBUFF_TAG, "Error receiving from queue");
            err = ESP_ERR_INVALID_ARG;
        }

        if (!err) {
            err = (rT.buff == NULL ? ESP_ERR_INVALID_ARG : ESP_OK);
        }
        ...
        if (!err) {
            ...
            {
                ESP_LOGE(CBUFF_TAG, "Error: Packets configured but incorrect read length");
                err = ESP_ERR_INVALID_ARG;
            }
            /* check overwrite status */
            else if (!buff->allow_overwrite && buffer_will_overwrite(buff, rT.read_len_bytes))
            {
                ESP_LOGE(CBUFF_TAG, "Error: New data would overwrite (which is not enabled)");
                err = ESP_ERR_INVALID_ARG;
            }
            /* check data size */
            else if (rT.read_len_bytes > buff->buffer_len)
            {
                ESP_LOGE(CBUFF_TAG, "Error: Data size larger than buffer");
                err = ESP_ERR_INVALID_ARG;
            }
        }
        ...
        if (!err) {
            ...
            if (err) {
                ESP_LOGE(CBUFF_TAG, "Error writing data to cbuffer [%u]", err);
            }
        }
    }
...
esp_err_t cbuffer_start_rx_task()
{
    ...
    if (xTaskCreate(cbuff_receive_task, "cbuff_receive_task", 2048, NULL, 5, &receiveTaskHandle)
        != pdTRUE)
    {
        ESP_LOGE(CBUFF_TAG, "Error creating receive task");
        return ESP_ERR_NO_MEM;
    }
...
esp_err_t cbuffer_read_i2c_block(
    CBuff handle,
    uint8_t bus,
    uint8_t device_addr,
    uint8_t reg_addr,
