 ...
 *           TODO: Add support for individual axis - make fifo_pkts.elements = 3 for each gyre and
 *accel Have to tweak the convert packet function
 *
 * \date     Aug 2020
 * \author   RJAM
 ****************************************/
...
const parameter_t lsm_parameter_map[lsm_param_map_len] = {
    ...
    {"Gyro Y", 2, &LSM_getGyroY, NULL, NULL, DATATYPE_FLOAT, 0, (GET_FLAG | STREAM_FLAG)},
    {"Gyro Z", 3, &LSM_getGyroZ, NULL, NULL, DATATYPE_FLOAT, 0, (GET_FLAG | STREAM_FLAG)},
    {"Accel X", 4, &LSM_getAccelX, NULL, NULL, DATATYPE_FLOAT, 0, (GET_FLAG | STREAM_FLAG)},
    {"Accel Y", 5, &LSM_getAccelY, NULL, NULL, DATATYPE_FLOAT, 0, (GET_FLAG | STREAM_FLAG)},
    {"Accel Z", 6, &LSM_getAccelZ, NULL, NULL, DATATYPE_FLOAT, 0, (GET_FLAG | STREAM_FLAG)},
    {"Op Mode",
     7,
     &LSM_getOpMode,
     &LSM_setOpMode,
    ...
    {"Accel Fullscale",
     10,
     &LSM_get_accel_full_scale,
     &LSM_set_accel_full_scale,
     NULL,
     DATATYPE_UINT8,
     LSM_ACCSCALE_8G,
     (GET_FLAG | SET_FLAG)},
    {"Gyro Fullscale",
     11,
     &LSM_get_gyro_full_scale,
     &LSM_set_gyro_full_scale,
     NULL,
     DATATYPE_UINT8,
    ...
    {"Accel BW mode",
     12,
     &LSM_get_accel_bw_select_mode,
     &LSM_set_accel_bw_select_mode,
     NULL,
     DATATYPE_UINT8,
     LSM_ACC_BW_FROM_REG,
     (GET_FLAG | SET_FLAG)},
    {"Gyro Sleep",
     13,
     &LSM_get_gyro_sleep_state,
     &LSM_set_gyro_sleep_state,
     NULL,
     DATATYPE_UINT8,
    ...
    {"Merge Interrupts",
     14,
     &LSM_get_int2_on_int1_state,
     &LSM_set_int2_on_int1_state,
     NULL,
     DATATYPE_UINT8,
     1,
     (GET_FLAG | SET_FLAG)},
    {"Temp pkt en",
     15,
     &LSM_get_temperature_pkt_en,
     &LSM_set_temperature_pkt_en,
     NULL,
     DATATYPE_BOOL,
     1,
     (GET_FLAG | SET_FLAG)},
    {"Threshold stop",
     16,
     &LSM_get_stop_on_fifo_thresh,
     &LSM_set_stop_on_fifo_thresh,
     NULL,
     DATATYPE_BOOL,
     1,
     (GET_FLAG | SET_FLAG)},
    {"Gyro HighPower",
     17,
     &LSM_get_gyro_hp_mode,
     &LSM_set_gyro_hp_mode,
     NULL,
     DATATYPE_BOOL,
     1,
     (GET_FLAG | SET_FLAG)},
    ...
    {"G HPass Cutoff",
     19,
     &LSM_get_gyro_hp_filter_cutoff,
     &LSM_set_gyro_hp_filter_cutoff,
     NULL,
     DATATYPE_UINT8,
    ...
    {"A LPass2 en",
     20,
     &LSM_get_accel_lp2_filter_en,
     &LSM_set_accel_lp2_filter_en,
     NULL,
     DATATYPE_BOOL,
     1,
     (GET_FLAG | SET_FLAG)},
    {"A HPass Cutoff",
     21,
     &LSM_get_accel_hpfilter_mode,
     &LSM_set_accel_hpfilter_mode,
     NULL,
     DATATYPE_BOOL,
     1,
     (GET_FLAG | SET_FLAG)},
    {"A HPass mode",
     22,
     &LSM_get_accel_hp_filter_cutoff,
     &LSM_set_accel_hp_filter_cutoff,
     NULL,
     DATATYPE_BOOL,
     1,
     (GET_FLAG | SET_FLAG)},
    {"6D LPass en",
     23,
     &LSM_get_lowpass_on_6d_mode,
     &LSM_set_lowpass_on_6d_mode,
     NULL,
     DATATYPE_BOOL,
     1,
     (GET_FLAG | SET_FLAG)},
    ...
    {"A HPass Cutoff",
     ...
     LSM_OPMODE_GYRO_ACCEL,
     (GET_FLAG | SET_FLAG)},
    {"Sample", 26, NULL, NULL, &LSM_sample_latest, DATATYPE_NONE, 0, (ACT_FLAG)},
    {"Reset Memory", 27, NULL, NULL, &LSM_reboot_memory, DATATYPE_NONE, 0, (ACT_FLAG)},
    {"Reset Device", 28, NULL, NULL, &LSM_reset_device, DATATYPE_NONE, 0, (ACT_FLAG)},
    {"Intr1 Triggers",
     29,
     NULL,
     &LSM_set_interrupt_1,
    ...
    {"BDU en",
     32,
     &LSM_get_block_data_update_en,
     &LSM_set_block_data_update_en,
     NULL,
     DATATYPE_BOOL,
     1,
     (GET_FLAG | SET_FLAG)},
    {"MSB only en",
     33,
     &LSM_get_only_msb_data,
     &LSM_set_only_msb_data,
     NULL,
     DATATYPE_BOOL,
     1,
     (GET_FLAG | SET_FLAG)},
    {"Step Pkt en",
     34,
     &LSM_get_fifo_step_en,
     &LSM_set_fifo_step_en,
     NULL,
     DATATYPE_BOOL,
     1,
     (GET_FLAG | SET_FLAG)},
    {"Step DRDY",
     35,
     &LSM_get_fifo_step_drdy,
     &LSM_set_fifo_step_drdy,
     NULL,
     DATATYPE_UINT8,
     1,
     (GET_FLAG | SET_FLAG)},
    ...
    {"Fifo Mode",
     ...
     NULL,
     DATATYPE_UINT8,
     LSM_FIFO_MODE_CONTINUOUS,
     (GET_FLAG | SET_FLAG)},
    {"Fifo Threshold",
     38,
     &LSM_get_fifo_watermark,
     &LSM_set_fifo_watermark,
     NULL,
     DATATYPE_UINT16,
     LSM_WATERMARK_MAX,
     (GET_FLAG | SET_FLAG)},
...
static void LSM_processAccel(LSM_DriverHandle_t *dev)
{
    ...
    int16_t s_ax =
        ((((int16_t)dev->measurements.rawAccel[1]) << 8)
         | ((int16_t)dev->measurements.rawAccel[0]));
    int16_t s_ay =
        ((((int16_t)dev->measurements.rawAccel[3]) << 8)
         | ((int16_t)dev->measurements.rawAccel[2]));
    int16_t s_az =
        ((((int16_t)dev->measurements.rawAccel[5]) << 8)
         | ((int16_t)dev->measurements.rawAccel[4]));
...
static void LSM_processGyro(LSM_DriverHandle_t *dev)
{
    ...
    int16_t x =
        ((((int16_t)dev->measurements.rawGyro[1]) << 8) | (int16_t)(dev->measurements.rawGyro[0]));
    int16_t y =
        ((((int16_t)dev->measurements.rawGyro[3]) << 8) | (int16_t)(dev->measurements.rawGyro[2]));
    int16_t z =
        ((((int16_t)dev->measurements.rawGyro[5]) << 8) | (int16_t)(dev->measurements.rawGyro[4]));
...
static esp_err_t process_interrupts(LSM_DriverHandle_t *dev, uint32_t sources)
{
    ...
    if (sources & LSM_NOTIFY_INTR_PIN_1) {
        if (dev->intr1_mask & LSM_INT1_TYPE_ACC_RDY || dev->intr1_mask & LSM_INT1_TYPE_GYR_RDY) {
            check_drdy = true;
        }
        ...
        if (dev->intr1_mask & LSM_INT1_TYPE_FIFO_THR || dev->intr1_mask & LSM_INT1_TYPE_FIFO_OVR
    ...
    if ((sources & LSM_NOTIFY_INTR_PIN_2)
        || ((sources & LSM_NOTIFY_INTR_PIN_1) && dev->settings.int2_on_int1))
    {
        if (dev->intr2_mask & LSM_INT2_TYPE_ACC_RDY || dev->intr2_mask & LSM_INT2_TYPE_GYR_RDY
            || dev->intr1_mask & LSM_INT2_TYPE_TMP_RDY)
        {
            check_drdy = true;
        }

        if (dev->intr1_mask & LSM_INT2_TYPE_FIFO_THR || dev->intr1_mask & LSM_INT2_TYPE_FIFO_OVR
            || dev->intr1_mask & LSM_INT2_TYPE_FIFOFULL)
        {
            check_fifo = true;
        }

        if (dev->intr1_mask & LSM_INT2_TYPE_STEPOVR || dev->intr1_mask & LSM_INT2_TYPE_STEPDELTA) {
            check_steps = true;
        }
    }
...
static float conversion_factor_accel(LSM_DriverHandle_t *dev)
{
    ...
    switch (dev->settings.accelScale) {
        case LSM_ACCSCALE_2G: factor = 0.061; break;
        case LSM_ACCSCALE_4G: factor = 0.122; break;
        case LSM_ACCSCALE_8G: factor = 0.244; break;
        case LSM_ACCSCALE_16G: factor = 0.488; break;
        default:
            factor = 0.0;
            ESP_LOGE(
                "LSM Driver",
                "Error: Unrecognised accelerometer setting [%u]",
                dev->settings.accelScale);
            break;
    }
...
static float conversion_factor_gyro(LSM_DriverHandle_t *dev)
{
    ...
    switch (dev->settings.gyroScale) {
        case LSM_GYRO_SCALE_125DPS: factor = 4.375; break;
        case LSM_GYRO_SCALE_250DPS: factor = 8.75; break;
        case LSM_GYRO_SCALE_500DPS: factor = 17.5; break;
        case LSM_GYRO_SCALE_1000DPS: factor = 35.0; break;
        case LSM_GYRO_SCALE_2000DPS: factor = 70.0; break;
...
static int16_t LSM_get_packet_length(LSM_DriverHandle_t *dev)
{
        ...
        && (dev->settings.accelRate > 0 && dev->settings.gyroRate > 0))
    {
        ...
        a_mod = (uint8_t)dev->settings.accelRate > (uint8_t)dev->settings.fifoODR
                    ? dev->settings.fifoODR
                    : dev->settings.accelRate;
        g_mod = (uint8_t)dev->settings.gyroRate > (uint8_t)dev->settings.fifoODR
                    ? dev->settings.fifoODR
                    : dev->settings.gyroRate;
                    ...
                    if (dev->status.fifo_full) {
                        ...
                        if (dev->use_events && dev->event_mask & LSM_EVENT_FIFO_FULL) {
                            err = esp_event_post_to(
                                ...
                                dev,
                                sizeof(LSM_DeviceSettings_t),
                                pdMS_TO_TICKS(LSM_CONFIG_EVENT_POST_MS));
                        }
#endif
...
#endif
{
    ...
    if (initData == NULL) {
        ...
        (initData->commMode == LSM_DEVICE_COMM_MODE_SPI && initData->commsChannel == 0)
        || initData->commsChannel > 2)
    {
    ...
    if (initStatus == ESP_OK) {
...
#ifdef CONFIG_DRIVERS_USE_HEAP
        LSM_DriverHandle_t *device = (LSM_DriverHandle_t *)
            heap_caps_calloc(1, sizeof(LSM_DriverHandle_t), MALLOC_CAP_8BIT);
        if (device == NULL) {
        ...
        if (initStatus == ESP_OK) {
            ...
            if ((initData->commMode == LSM_DEVICE_COMM_MODE_SPI
                 || initData->commMode == LSM_DEVICE_COMM_MODE_SPI_3))
            {
                /** TODO: setup spi  **/
                return NULL;
        ...
        if (initStatus == ESP_OK && initData->use_cbuffer && initData->cbuff != NULL) {
            ...
            if (dmaMem < LSM_FIFO_BUFFER_MEM_LEN) {
                ESP_LOGE(
                    "LSM Driver",
                    "Error: insufficient DMA cap mem. Only %d bytes available",
                    dmaMem);
                initStatus = ESP_ERR_NO_MEM;
            } else {
        ...
        if (initStatus == ESP_OK) {
            ...
            if (initStatus == ESP_OK && who != LSM_WHOAMI) {
                ESP_LOGE(
                    "LSM Driver",
                    "Error - invalid device id! Expected %02x, Recevied: %02x",
                    LSM_WHOAMI,
                    who);
        ...
        if (initStatus == ESP_OK) {
            ...
            if (xTaskCreate(LSMDriverTask, "LSMDriverTask", 5012, (void *)device, 3, &taskHandle)
                == pdFALSE) {
    ...
    {
        uint8_t regval = 0;
        esp_err_t err = gcd_i2c_read_address(
            dev->commsChannel,
            dev->devAddr,
            LSM_CTRL3_C_REG,
            1,
    ...
    {
        /** copying these from the appnote diagram - see there for explanation of commands (see
         * above for app note) **/
        uint8_t regval = 0x30;
        uint8_t abort = 0;
        uint8_t temp_data[6];
        ...
        esp_err_t err = gcd_i2c_write_address(
            dev->commsChannel,
            dev->devAddr,
            LSM_CTRL1_XL_REG,
            1,
        ...
        } /** appnote says something about... reading status bit 5 times? Surely just waiting for
             new data? **/
        ...
        ESP_LOGI(
            "LSM Driver",
            "Got Pre-SelfTest Data: x - %u y - %u z - %u",
            x_no_st,
            y_no_st,
            z_no_st);
        ...
        ESP_LOGI(
            "LSM Driver",
            "Got the selfTest measurements: x - %u y - %u z - %u",
            x_st,
            y_st,
            z_st);

        /** now, what the hell are the values for min(ST_X) and max(ST_X) ???? ?!??!???!?!??!?
         * dammit ST why make this so difficult? **/
    ...
    esp_err_t LSM_set_poll_sample_period_ms(LSM_DriverHandle_t * dev, uint16_t * val)
    {
        ...
        if (v > (uint16_t)LSM_DRIVER_MAX_SAMPLE_PERIOD_MS
            || v < (uint16_t)LSM_DRIVER_MIN_SAMPLE_PERIOD_MS) {
    ...
    esp_err_t LSM_setOpMode(LSM_DriverHandle_t * dev, LSM_OperatingMode_t * mode)
    {
        ...
        if (m == LSM_OPMODE_ACCEL_ONLY) {
            writeVals[0] =
                ((LSM_CTRL9_ACCEL_Z_EN_BIT) | (LSM_CTRL9_ACCEL_Y_EN_BIT)
                 | (LSM_CTRL9_ACCEL_X_EN_BIT) | (regVals[0] & 0b111));
        } else if (m == LSM_OPMODE_GYRO_ONLY) {
            writeVals[1] =
                ((LSM_CTRL10_GYRO_Z_EN_BIT) | (LSM_CTRL10_GYRO_Y_EN_BIT)
                 | (LSM_CTRL10_GYRO_X_EN_BIT) | (regVals[1] & 0b111));
    ...
    esp_err_t LSM_set_accel_bw_select_mode(
    ...
    esp_err_t LSM_get_accel_bw_select_mode(
        LSM_DriverHandle_t * dev,
        LSM_AccelBandwidthMode_t * mode)
    {
        esp_err_t status = ESP_OK;
    ...
    esp_err_t LSM_set_gyro_hp_filter_cutoff(
        LSM_DriverHandle_t * dev,
        LSM_GyroHighpassCutoff_t * cutoff)
    {
        esp_err_t err = ESP_OK;
        uint8_t m = *cutoff;
        uint8_t regval = 0;
    ...
    esp_err_t LSM_get_gyro_hp_filter_cutoff(
        LSM_DriverHandle_t * dev,
        LSM_GyroHighpassCutoff_t * cutoff)
    {
        esp_err_t status = ESP_OK;
    ...
    esp_err_t LSM_set_accel_hp_filter_cutoff(
        LSM_DriverHandle_t * dev,
        LSM_HighpassSlopeSettings_t * cutoff)
    {
        esp_err_t err = ESP_OK;
        uint8_t m = *cutoff;
        uint8_t regval = 0;
    ...
    esp_err_t LSM_get_accel_hp_filter_cutoff(
        LSM_DriverHandle_t * dev,
        LSM_HighpassSlopeSettings_t * cutoff)
    {
        esp_err_t status = ESP_OK;
    ...
    esp_err_t LSM_update_temperature(LSM_DriverHandle_t * dev)
    {
        ...
        status = gcd_i2c_read_address(
            ...
            LSM_TEMP_LSB_REG,
            2,
            &dev->measurements.raw_temp[0]);

        if (!status) {
            dev->measurements.calibTemperature =
                (((int16_t)dev->measurements.raw_temp[1] << 8)
                 | (int16_t)dev->measurements.raw_temp[0]);
        }
    ...
    esp_err_t LSM_set_fifo_odr(LSM_DriverHandle_t * dev, uint8_t * odr)
    {
        ...
        else
        {
            ...
            if (o < ar || o < gr) {
                ESP_LOGI(
                    "LSM_Driver",
                    "Invalid ODR: FIFO data rate is > accel/gyro data rate - fifo will be "
                    "automatically limited to slowest data rate g[%u] a[%u] o[%u]",
                    gr,
                    ar,
                    o);
            }
    ...
    esp_err_t LSM_set_fifo_watermark(LSM_DriverHandle_t * dev, uint16_t * watermark)
    {
        ...
        if (val > LSM_WATERMARK_MAX) {
            ESP_LOGE(
                "LSM_Driver",
                "Error - max watermark exceeded %u (max: %u)",
                val,
                LSM_WATERMARK_MAX);
            status = ESP_ERR_INVALID_ARG;
        } else {
            ...
            esp_err_t status = gcd_i2c_read_address(
                dev->commsChannel,
                dev->devAddr,
                LSM_FIFO_CTRL2_REG,
                1,
    ...
    esp_err_t LSM_setFIFOpackets(LSM_DriverHandle_t * device, LSM_PktType_t * pT)
    {
        ...
        if ((uint8_t)device->settings.accelRate != (uint8_t)device->settings.gyroRate) {
            ESP_LOGI(
                "LSM_Driver",
                "Gyro / Accel ODRs are not equal - driver will be unable to track packets "
                "accurately!");
        }
        ...
        if (pktType & LSM_PKT4_STEP_TS && LSM_PKT4_TEMP) {
            ESP_LOGE(
                "LSM_Driver",
                "Error: Cannot set both temperature and pedometer simultaneously");
        ...
        if (status == ESP_OK) {
            ...
            ESP_LOGI(
                "LSM_Driver",
                "Succesfully updated fifo packets! Elements: %u Raw len: %u Proc Len: %u",
                device->fifo_pkts.pkt_elements,
    ...
    esp_err_t LSM_read_fifo_to_cbuffer(LSM_DriverHandle_t * dev)
    {
        ...
        if (dev->cbuff_store_packets) {
            ...
            /** check how many full packets there are - leave 1 full packet in fifo if 'stop on
             * thresh' enabled **/
            ...
            for (int j = 0; j < full_packets; j++) {
                ...
                if (!dev->cbuff_store_raw) {
                    ...
                    err = cbuffer_write_packet(
                        dev->cbuff,
                        processed,
                        dev->fifo_pkts.pkt_len_post_proc);
                } else {
    ...
    esp_err_t LSM_readFifoBlock(LSM_DriverHandle_t * dev, uint16_t * num_samples)
    {
        ...
        if (!status && dev->use_cbuffer) {
            ...
            /** read an entire set of apckets at a time - ideally, fifo pkt index should start at n
             * and and at n-1 **/
