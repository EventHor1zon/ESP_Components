...
#define CONFIG_APDS_TASK_STACK_SZ 5012
...
typedef struct APDS_GeneralSettings {
    ...
    uint32_t wait_time_ms; /** time to idle per cycle in ms(see datasheet for
                              state machine) **/
...
typedef struct APDS9960_Driver {
    ...
    uint8_t missed_isr;  /* interrupts missed */
...
TaskHandle_t apds_driver_task_h;
...
else APDS_DEV apds_init(APDS_DEV dev, apds_init_t *init);
#endif
 ...
 *          set to 0xFF at power-on. Saturation value = 1-25 * CYCLES (max
 *65535) register value 0 - 256 cycles ~ 712ms = 256 - VALUE / 2.78 ms 255 - 1
 *cycle ~ 2.78ms \param dev - apds handle \param adct - pointer to value storage
 * \return ESP_OK or error
 **/
